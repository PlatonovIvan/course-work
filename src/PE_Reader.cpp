...
void Raw_Reader::LoadForStaticAnalise(char* &buf, unsigned int& len, 
												unsigned int& start){	
...
void Raw_Reader::LoadForDynamicAnalise(char* &buf, unsigned int& len, 
...
void PE_Reader::LoadForStaticAnalise(char* &buf, unsigned int& len, unsigned int& start){	
	PE_HeaderOffset();
	PE_FileType();
	PE_TableOffset();
	PE_NumberOfEntries();
	...
	if (len!=0)
		...
		printf("There are no code section in this file\n");
		exit(1);
	}	
	if (buf==NULL){
		printf("Memory error in static analise\n"); 
...
void PE_Reader::LoadForDynamicAnalise(char* &buf, unsigned int& len, unsigned int & start, unsigned int &base, unsigned int & offset){	
	PE_HeaderOffset();
	PE_FileType();
	PE_TableOffset();
	PE_NumberOfEntries();
