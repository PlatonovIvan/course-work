...
void Emulator_LibEmu::begin(char* & buf, const unsigned int len, const unsigned int entry_point, const unsigned int base){
	im_base=base;
	for (int i=0; i<8; i++){
...
void Emulator_LibEmu::jump(const unsigned int pos){
...
void Emulator_LibEmu::get_command(char *buff, const unsigned int size) const{
	emu_memory_read_block(mem, emu_cpu_eip_get(cpu), buff, size);
}

void Emulator_LibEmu::read_block(char *buff, const unsigned int size) const{
